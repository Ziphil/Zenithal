...
ZenML also supports character references, but only in the hexadecimal representation.
The code point following `&#x` is case-insensitive.
For example, `&#xA3>` or `&#xa3>` represents `£`, while `&#163>` is invalid.
