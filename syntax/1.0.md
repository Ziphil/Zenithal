...
### Macro
If the element name is preceded by `&`, instead of `\`, the element is treated as a macro.

Entity references are implemented as macros which are each converted to a text node containing a single character.
